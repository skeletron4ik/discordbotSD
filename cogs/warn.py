    ...
    async def warn(self, inter: disnake.ApplicationCommandInteraction, участник: disnake.Member, количество: int,
        ...
        if inter.type == disnake.InteractionType.application_command:
            ...
            if количество < 1:  # Проверка на долбоеба
                ...
                await inter.edit_original_message(embed=exceptWarn, ephemeral=True)
                return
            ...
            def get_warning_word(warnings):
                if warnings % 10 == 1 and warnings % 100 != 11:
                    return "предупреждение"
                elif 2 <= warnings % 10 <= 4 and not (12 <= warnings % 100 <= 14):
            ...
            embed = disnake.Embed(
                description=f"Участник {участник.mention} получил ``{количество}`` {warning_word} (Всего:{warns_count}) на ``{HasRole}``, по причине: **{reason}.**",
                colour=0xffff00,
            ...
            embed.set_footer(text=f"Случай: #{server_value}")
            ...
            if warns_count == 2:
                ...
                embed.add_field(name="Причина:", value="Количество полученых предупреждений превышает: 10",
                                inline=False)
    ...
    async def unwarn(self, inter: disnake.ApplicationCommandInteraction, участник: disnake.Member,
        ...
        if inter.type == disnake.InteractionType.application_command:
            ...
            if collusers.count_documents({"id": участник.id, "guild_id": inter.guild.id,
                ...
                embed.set_author(name="Предупреждение было успешно снято!",
                                 icon_url="https://www.emojiall.com/images/240/telegram/2705.gif")

                embed.add_field(name="Снятие предупреждения:",
                                value=f"{inter.author.mention}, Вы успешно сняли предупреждение #{предупреждение} с участника {участник.name}, на сервере {inter.guild.name}.",
                                inline=False)
                ...
                embed.set_author(name="Ваше предупреждение было снято!",
                                 icon_url="https://www.emojiall.com/images/240/telegram/2705.gif")

                embed.add_field(name="Снятие предупреждения:",
                                value=f"Предупреждение #{предупреждение} на сервере {inter.guild.name} было снято Модератором: {inter.author.name} ({inter.author.mention})!",
                                inline=False)
                ...
                embed.add_field(name="", value=f"Предупреждение было снято.",
                                inline=False)
